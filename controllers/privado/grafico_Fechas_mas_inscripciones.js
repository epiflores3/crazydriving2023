...
const FECHA1 = document.getElementById("inicial");
const FECHA2 = document.getElementById("final");
...
SAVE_FORM.addEventListener('submit', async (event) => {
    ...
    const JSON = await dataFetch(INSCRIPCION_API, 'inscripcionesMasFechas', FORM);
    ...
    if (JSON.status) {
        ...
        JSON.dataset.forEach(row => {
            ...
            fechas.push(row.fecha_registro);
        ...
        barGraph('chart2', fechas, cantidad, 'Cantidad de inscripciones', 'Top 5 de fechas con más inscripciones');
...
function deshabilitarBoton(){
    TEXT_1 = document.getElementById("inicial").value;
    TEXT_2= document.getElementById("final").value;
    VAL = 0;
   ...
   document.getElementById("inicial").addEventListener("keyup", deshabilitarBoton);
   document.getElementById("final").addEventListener("keyup", deshabilitarBoton);
