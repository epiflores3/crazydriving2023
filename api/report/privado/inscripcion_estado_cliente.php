...
if ($estados_inscripcion = $inscripcion::ESTADOS) {
    ...
    foreach ($estados_inscripcion as $estado) {
        // Se muestra la celda que tendra el dato por el que se filtra.
        $pdf->cell(0, 10, $pdf->encodeString('Estado: ' . $estado[0]), 1, 1, 'C', 1);

        // Se establece por el id que tiene que capturar.
        if ($inscripcion->setEstado($estado[0])) {
            // Verifica si exiten registros a mostrar.
            if ($data_inscripcion = $inscripcion->inscripcionEstadoCliente()) {
                // Recorre filas una por una.
                foreach ($data_inscripcion as $row_inscripcion) {
                    $row_inscripcion['evaluacion'] ? $evaluacion = 'Evaluado' : $evaluacion = 'No evaluado';
                    // Se rellenan las celdas de acuerdo a un producto en especifico.
                    $pdf->cell(70, 10, $pdf->encodeString($row_inscripcion['nombre_com_cliente']), 1, 0);
                    $pdf->cell(20, 10, $pdf->encodeString($row_inscripcion['anticipo_paquete']), 1, 0);
                    // $pdf->cell(30, 10, $pdf->encodeString($rowEstado2['fecha_registro']), 1, 0);
                    $pdf->cell(28, 10, $pdf->encodeString($row_inscripcion['fecha_inicio']), 1, 0);
                    $pdf->cell(20, 10, $pdf->encodeString($evaluacion), 1, 0);
                    // $pdf->cell(25, 10, $pdf->encodeString($rowEstado2['descripcion']), 1, 0);

                    $pdf->cell(62, 10, $pdf->encodeString($row_inscripcion['nombre_com_empleado']), 1, 0);
                    $pdf->cell(18, 10, $pdf->encodeString($row_inscripcion['inicio']), 1, 0);
                    $pdf->cell(18, 10, $pdf->encodeString($row_inscripcion['fin']), 1, 1);
                }
            } else {
